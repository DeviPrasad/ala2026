...
\date{\center{Wednesday, 5 August 2026}}
\maketitle
...
\begin{frame}\FrameTitle{Unit Vector}{Unit 1}
    ...
    with \emph{i}th element one, and denoted $e_i$.
...
\begin{frame}\FrameTitle{Unit Vector}{Unit 1}
    ...
    $e_i$ is an \emph{n}-vector.
    $(e_i)_j$ is a number, its \emph{j}th entry.
\end{frame}

\begin{frame}\FrameTitle{Ones Vector}{Unit 1}
    A \emph{ones vector} is a vector with all elements equal to one.
    \vsp
    The \emph{n}-dimensional ones vector is denoted $1_n$.
        ...
        \begin{column}{0.8\textwidth}
            ...
            \begin{tikzpicture}[scale=0.8]
                ...
                \draw[thick, -stealth, orange] (1,1) -- (2,5.5)
                    node[midway, left, orange] {$\vec{u} + \vec{v}$};
            \end{tikzpicture}
            \end{center}
        \end{column}
    \end{columns}
...
\begin{frame}\FrameTitle{Linear Combinations}{Unit 1}
    if $a_1, \ldots, a_m$ are \emph{n}-vectors, and $\beta_1,\ldots,\beta_m$ are scalars,
    the \emph{n}-vector
    \[
        \beta_1 a_1 + \cdots + \beta_m a_m
    \]
    is called the \emph{linear combination} of the vectors $a_1,\ldots,a_m$.
...
\begin{frame}\FrameTitle{Inner Product}{}
    \begin{definition}[Inner or Dot Product]
        ...
        \[
            a^Tb = a_1b_1 + a_2b_2 + \ldots + a_nb_n,
        \]
        the sum of the products of corresponding entries.
    \end{definition}
    \vsp
    If a, b, and c are vectors of the same size, and {\color{yellow}$\gamma$} is a scalar, then the following hold:\vspace*{2pt}\\
    \begin{enumerate}
        \item \emph{Commutativity}: $a^{\textrm{T}}b = b^{\textrm{T}}a$ aka \emph{symmetric}\vspace*{3pt}\\
        \item \emph{Associativity with scalar multiplication}: $({\color{yellow}\gamma} a)^{\textrm{T}}b = {\color{yellow}\gamma} (a^{\textrm{T}}b)$ \vspace*{3pt}\\
        \item \emph{Distributivity with vector addition}: $(a+b)^{\textrm{T}}c = a^{\textrm{T}}c + b^{\textrm{T}}c$
    \end{enumerate}
\end{frame}

\begin{frame}\FrameTitle{Inner Product: Special Examples}{Unit 1}
    \begin{itemize}
        \item \emph{Unit vector}. $e^{\textrm{T}}_i a = a_i$
        \item \emph{Sum}. $1^{\textrm{T}} a= a_1 + \cdots + a_n$
        \item \emph{Sum of squares}. $a^{\textrm{T}} a = a^2_1 + \cdots + a^2_n$
    \end{itemize}
\end{frame}

\begin{frame}\FrameTitle{Inner Product: Applications}{Unit 1}
    \begin{itemize}
        \item \emph{Co-occurrence}. $a^{\textrm{T}} b = a_1b_1 + \cdots + a_nb_n$ counts the number of times
        \item \emph{Weighs,features, and scores}. $w^{\textrm{T}} x = w_1x_1 + \cdots + w_nx_n$
...
\begin{frame}\FrameTitle{Outer Product}{}
    \begin{definition}[Outer Product]
        The outer product of a \emph{m}-vector \emph{a} and an \emph{n}-vector \emph{b} is
        given by $ab^T$, which is an m×n matrix
        \[
            ab^T = \begin{bmatrix}
    ...
    \vsp
    In general, $a^{\textrm{T}}b \neq b^{\textrm{T}}a$, \emph{i.e.},
