...
\begin{frame}\FrameTitle{Inner Product}{}
    ...
    \end{definition}
    \vsp
    If a, b, and c are vectors of the same size, and $\gamma$ is a scalar, then the following hold:\vspace*{2pt}\\
    1. \emph{Commutativity}: $a^{\textrm{T}}b = b^{\textrm{T}}a$ \vspace*{3pt}\\
    2. \emph{Associativity with scalar multiplication}: $(\gamma a)^{\textrm{T}}b = \gamma (a^{\textrm{T}}b)$ \vspace*{3pt}\\
    3. \emph{Distributivity with vector addition}: $(a+b)^{\textrm{T}}c = a^{\textrm{T}}c + b^{\textrm{T}}c$ \\
\end{frame}
